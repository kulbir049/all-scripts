...
$search = $_SESSION["tag_search"];

$sq = "SELECT sweeb_id FROM tags WHERE tag LIKE '%$search%'";
$resul = $conn->query($sq);

if ($resul->num_rows > 0) {
    // output data of each row
    while($ro = $resul->fetch_assoc()) {
  $sweeb_tag_id = $ro['sweeb_id'];
  
  
//throw into tag db
$tags = array($sweeb_tag_id);
$boom = implode(", ",$tags);
$boom = ltrim ($boom, ',');



$friends = explode(",", $boom);
$sql = "SELECT *  FROM sweebs WHERE status='active' AND id IN (" . implode(",", array_map("intval", $friends)) . ") ORDER BY id DESC LIMIT ".$postnumbers." OFFSET ".$offset."";
