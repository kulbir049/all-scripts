...
class Admin_categories extends MY_Controller
{
    ...
    public function index()
    {
        ...
        //$data['objTree'] = $this->Category_model->getCategoryTreeData_new();
        //dd($data['objTree']); die;
    ...
    public function deleted_data()
    {
        ...
        $this->breadcrumbs->push('Master Composers', '/admin/category');
        $data['breadcrumb'] = $this->breadcrumbs->show();
        $data['objTree'] = $this->Category_model->getCategoryTreeData_deleted_data($data['parent_id']);
        //dd($data['objTree']); die;
        $data['title'] = 'Manage Libraries';
        _adminLayout('categories_delete', $data);
    }
    public function ajax_root_folder_deleted_data()
    {
        //['1','2','132','1321','12522','12619','13763','14155']
        $parent_id = ['1', '2', '132', '1321', '12522', '12619', '13763', '14155'];
        $verify_folders = array();
        $folders = $this->Category_model->getCategoryTreeData_deleted_data($parent_id);
        foreach ($folders as $value) {
            $verify = $this->Category_model->getCategoryTreeData_deleted_verify($value->parent_url);
            if (count($verify) == 0) {
                $verify_folders[] = $value;
            }
        }

        $data['objTree']['folders'] = $verify_folders;
        $data['objTree']['files'] = '';

        echo  json_encode($data['objTree']);
        die;
    }
    ...
    public function ajax_root_folder_index_gaurav()
    {
        ...
        $files = $this->Category_model->getImagesById($data['parent_id']);
        $data['objTree']['folders'] = $folders;
        if ($data['redirect_type'] == 'sd') {
    ...
    function ajax_root_folder_index_gaurav123()
    {
        ...
        $data['redirect_type'] = $this->input->post('redirect_type'); //'mc';
        $alphaSearch_keyword = $this->input->post('alphaSearch_keyword'); //'mc';
        $keywords = $this->input->post('keywords123');
        ...
        $data['objTree_1'] = $this->Category_model->get_keywordsearch_file(strtolower($keywords), $parent_id);
        $control_id = 0;
        foreach ($data['objTree_1'] as $key => $value) {
            $root_ids_folders = $this->Category_model->getDirNamesById_all_parent_folders($value->id, 0);
            foreach ($root_ids_folders as $key => $value) {
                $filter_all_parent_ids[] = $value;
            }
        }

        $this->session->set_userdata('root_folders_ids', array_unique($filter_all_parent_ids));
        foreach ($data['objTree_1'] as $key => $value) {
            $root_ids[] = $this->Category_model->getDirNamesById_keywordsearch($value->id, $control_id);
        }
        // echo "<pre>";print_r($root_ids);die;
        foreach ($root_ids as $key => $value) {

            if ((int)$value[0] == $parent_id || (int)$value[1] == $parent_id || (int)$value[2] == $parent_id || (int)$value[3] == $parent_id || (int)$value[4] == $parent_id || (int)$value[5] == $parent_id || (int)$value[6] == $parent_id || (int)$value[7] == $parent_id || (int)$value[8] == $parent_id) {
                $start_key = (int)array_search($parent_id, $value);
                $a = (int)$value[$start_key];
                $b = (int)$value[$start_key + 1];
                $c = (int)$value[$start_key + 2];
                $d = (int)$value[$start_key + 3];
                $root_path_array[$a][] = $b;
            }
        }
        //  echo "<pre>";echo "gaurav";echo($parent_id);print_r($root_path_array[$parent_id]);die;
        $root_path_array_filter = array();

        foreach ($root_path_array[$parent_id] as $key => $value) {
            if ($value > 0) {
                $root_path_array_filter[] = $value;
            }
        }
        $search_list_ids_key = array_search($parent_id, $search_list_ids);
        $folder_id = '';
        if (array_key_exists(($search_list_ids_key + 1), $search_list_ids)) {
            $folder_id = $search_list_ids[($search_list_ids_key + 1)];
        }

        //print_r($root_path_array[$parent_id]);
        //if($ajax_search_file!='' || $ajax_search_file){
        if (count($root_path_array[$parent_id]) == 1 && $root_path_array[$parent_id][0] == 0) {
            unset($root_path_array[$parent_id][0]);
        }
        //print_r($root_path_array[$parent_id]);
        //$data['objTree'] = $this->Comp_model->get_keywordsearch($ajax_search,$ajax_search_file,$parent_id,$folder_id);
        if (count($root_path_array[$parent_id])) {
            $data['objTree'] = $this->Category_model->get_folders_by_ids($root_path_array_filter, $parent_id);
        } elseif (count($root_path_array[$parent_id]) == 0 && ($file_folder_search == 0 && $only_file_search == 0)) {
            $data['objTree'] = $this->Category_model->getCategoryTreeData_ajax($parent_id);
        }
        if ($data['ajax_search_file'] != '') {
            $data['verify_folder_name'] = $this->Category_model->get_keywordsearch_verify(strtolower($data['ajax_search_file']), $parent_id);
            //print_r($data['verify_folder_name']);
            $this->session->set_userdata('verify_folder_name_id', $data['verify_folder_name'][0]->id);
        } else {
            $data['verify_folder_name'] = array();
        }
        //print_r($data['verify_parent_name'][0]->parent_id);//die;
        if (count($data['verify_folder_name']) > 0 && $data['ajax_search_file'] != '') {
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 1);
        } elseif ($data['verify_parent_name'][0]->parent_id == 1 || $data['verify_parent_name'][0]->parent_id == 132 || $data['verify_parent_name'][0]->parent_id == 1321 || $data['verify_parent_name'][0]->parent_id == 13763) {
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
        } elseif ($data['search_level'] < $this->session->userdata('searching_level')) {
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
        }

        //$this->session->set_userdata('searching_level',$data['search_level']);
        // echo "<pre>";print_r($root_path_array_filter);die;
        $data['objTree']['count'] = 0;
        $data['objTree']['next_data'] = 0;
        $data['objTree']['files'] = $this->Category_model->getImagesById11($parent_id, $ajax_search_file);
        if ($parent_id == 132) {
            //   die;
            $data['objTree']['folders'] = $this->Category_model->get_folders_by_idsC($root_path_array_filter, ["1", "132"]);
        } else {
            $data['objTree']['folders'] = $this->Category_model->get_folders_by_ids($root_path_array_filter, $parent_id);
        }
        if (($this->session->userdata('ajax_search_file_folder_string_matched') > 0) && ($parent_id != 1 && $parent_id != 132 && $parent_id != 1321 && $parent_id != 12522 && $parent_id != 12619)) {
            // echo "hello";
            $data['objTree']['folders'] = $this->Category_model->getCategoryTreeData_ajax1($parent_id);
        }
        $data['objTree']['folders1'] = $this->session->userdata('ajax_search_file_folder_string_matched');
    ...
    public function rename_folder()
    {
        ...
        $this->session->set_userdata('destination_last_id', $this->input->post('id'));
    ...
    public function permanent_delete_category()
    {
        ...
        if ($id > 0) {
            ...
            $add_dir_path = $this->Category_model->getDirNamesById($id);
            $dst = FCPATH . "assets/uploads/Sheet-Music/" . $add_dir_path;
            $folder_lenght = explode('/', $dst);
            // dd(count($folder_lenght));
            // dd($folder_lenght[8]);
            // dd($dst);
            // die;
            if (count($folder_lenght) > 7 && !empty($folder_lenght[8])) {
                if (file_exists($dst) && is_dir($dst)) {
                    permanentDeleteFolder($dst);
                }
            }
            // dd($dst);

            $add_dir_path = $this->Category_model->update_del_status($id, $delete_status);
            $this->sub_cat_delete_records();
            ...
            if (isPostBack()) {
                ...
                if ($_FILES['fileUpload']['name'] != '') {
                    ...
                    if ($post[parent_id] == 'Select Composer') {
                        ...
                        $_SESSION['tyoe_smi_operation'] = 'error';
                    ...
                    if ($post['replace_file_id'] > 0) {
                        $add_file_path_new = $this->Category_model->get_file_detail($post['replace_file_id']);
                        $new_name = $add_file_path_new->image;

                        //  echo FCPATH.$dst.'/'.$new_name;die;


                        unlink(FCPATH . $dst . '/' . $new_name);
                    ...
                    if (!$this->upload->do_upload('fileUpload')) {
                        ...
                        if ($post['replace_file_id'] > 0) {
                            $add_file_path_new = $this->Category_model->get_file_detail($post['replace_file_id']);
                            $save_data['image'] = $add_file_path_new->image;
                            //print_r($post);   print_r($dst); print_r($save_data);die;
    ...
    public function add_directory_ajax()
    {
        ...
        if (isPostBack()) {
            ...
            // echo $maxUpload."<br>".$maxPost."<br>";

            /*$maxsize=1024 * 2;
if(($_FILES['fileUpload']['size'] >= $maxsize)) {
       // $errors[] = 'File too large. File must be less than 2 megabytes.';
        // echo "<span style='color:red;'>File too large. File must be less than 2 megabytes.</span>";
        
       $this->session->set_flashdata('error','File too large. File must be less than 2 megabytes.');
         die;
    }
*/
            ...
            $post = $this->input->post();
            if (isset($post['common'])) {
                $this->session->set_userdata('common', $post['common']);
                unset($post['common']);
            }
            unset($post['action_taken']);
            // echo "<pre>";print_r($_FILES);print_r($post);die;
            ////////////////////////////
            if ($_FILES['fileUpload']['name'] != '') {
                $filename = $_FILES['fileUpload']['name'];
                $file_name = str_replace('.zip', '', $filename);
                        ...
                        function scanFolder($dir, $parent)
                        {
                            ...
                            if (is_dir($dir)) {
                                ...
                                foreach ($scn_dir as $value) {
                                    if (!in_array($value, array(".", "..", "_MACOSX"))) {
            ...
            if (isPostBack()) {
                ...
                if ($_FILES['fileUpload']['name'] != '') {
                    ...
                    if ($post['parent_id'] == 'Select Composer') {
                        ...
                        //$cat_name = $this->Category_model->getRecordById($post[parent_id]);
                        $add_dir_path = $this->Category_model->getDirNamesById($post['parent_id']);
                        //  print_r($add_dir_path); die; 
                        $destination_ids = $this->Category_model->get_destination_ids($post['parent_id']);
                        //  print_r($destination_ids); die;
                        $destination_ids[] = $post['parent_id'];
                        $this->session->set_userdata('destination_ids', $destination_ids);
                        $this->session->set_userdata('destination_last_id', $post['parent_id']);
                        $this->session->set_userdata('main_root_ids', $post['parent_id']);
    ...
    public function replace_file_content($id = false)
    {
        ...
        _adminLayout('replace_file_content', $data);
    }
...
function moveDirectory($source, $destination) {
    ...
    while (($file = readdir($dir)) !== false) {
        ...
        if (is_dir($sourcePath)) {
            moveDirectory($sourcePath, $destinationPath);  // Recursively move subdirectories
        } else {
    ...
    // Optionally, remove the source directory if it is now empty
    rmdir($source);
}
...
// ini_set('display_errors', 1);
        $data['type'] = 'file';
        $data['action'] = 'move_folders';
        $data['submit_button_name'] = 'Move folders';
        $data['back_redirect_btn'] = $this->uri->segment(5);
        //echo "<pre>";print_r($this->input->post());die;
        if (isPostBack()) {
            ...
            if ($this->input->post("move")) {
                ...
                function rcopy($dir, $dst)
                {
                    // //if (file_exists ( $dst ))
                    // //  rrmdir ( $dst );
                    // dd($dir); dd(is_dir($dir));dd(mkdir($dst));dd($dst);die;
                    // if (is_dir($dir)) {
                    //     mkdir($dst);
                    //     $files = scandir($dir);
                    //     foreach ($files as $file)
                    ...
                    $sourceDir = $dir;
                    $folder_name = end(explode('/', $dir));
                    $destinationDir = $dst.'/'.$folder_name;
                    

                    if (is_dir($sourceDir)) {
                        // Ensure the destination directory exists, create it if not
                        if (!is_dir($destinationDir)) {
                            mkdir($destinationDir, 0755, true);  // Creates the directory if it doesn't exist
                        }
            ...
            // Optionally, remove the empty source directory after moving its contents
                ...
                $move_folder_ids=explode(',',$post['ids']);
                dd($move_folder_ids);
                
                foreach($move_folder_ids as $id){
                    
                $dir_idm = $this->Category_model->getDirNamesById($id);
                
                $folder_name = explode('/', $dir_idm);
                //  var_dump($this->input->post('hidden_select_radio1'));

               
                //   var_dump($id."---".$post['hidden_select_radio1']); die('aa');
                $dest_idm = $this->Category_model->getDirNamesById($post['hidden_select_radio1']);
                $this->session->set_userdata('destination_last_id', $post['hidden_select_radio1']);
                $this->session->set_userdata('gaurav_replace', false);
                ...
                // if($id==79206){
                //     print_r($dir);print_r('<br/>');print_r($dst);die;
                // }
        ...
        _adminLayout('move-approved-folders-gaurav', $data);
    }
    ...
    public function copy_directory($id = false)
    {
        ...
        try {
            ...
            if (isPostBack()) {
                ...
                // echo "<pre>";echo "<hr>";print_r($desired_id);echo "<hr>"; print_r($dest_idm);echo "<hr>";
                ...
                // print_r($categories);echo "<hr>";echo "<hr>";
                function filderImageCopier($id, $last_ins_id3)
                {
                    $CI = &get_instance();
                    $CI->db->select('*');
                    $CI->db->from('category_image');
                    $CI->db->where("cat_id", $id);
                    $parent1 = $CI->db->get();
                    $categories1 = $parent1->result();
                    //   print_r($categories1);echo "<hr>";
                    foreach ($categories1 as $ar) {
                        $a = [];

                        $a['cat_id'] = $last_ins_id3;
                        $a['image'] = $ar->image;
                        $a['file_user_id'] = $ar->file_user_id;
                        $a['rename_file'] = $ar->rename_file;
                        $a['custom_text'] = $ar->custom_text;
                        $a['file_path'] = $ar->file_path;
                        $a['status'] = $ar->status;
                        $a['delete_status'] = $ar->delete_status;
                        $a['keyword_text'] = $ar->keyword_text;
                        $a['sale_price'] = $ar->sale_price;
                        $a['is_personal_library'] = $ar->is_personal_library;
                        $a['personal_library_folder'] = $ar->personal_library_folder;
                        $CI->db->insert('category_image', $a);
                        // $last_ins_id= $CI->db->insert_id();
                    }
                }
                function copy_folder_database_data_gaurav($asc, $last_ins_id1)
                {
                    //  echo "<pre>";echo ($asc);
                    if ($asc != "") {
                        // echo "<br><hr>File:-<br><hr>";
                        filderImageCopier($asc, $last_ins_id1);
                        // echo "<hr><hr>";
                        /// echo "<br><hr>folder:-";
                        $CI = &get_instance();
                        $CI->db->select('*');
                        $CI->db->from('categories');
                        $CI->db->where("parent_id", $asc);
                        $parent1 = $CI->db->get();
                        $categories1 = $parent1->result();
                        //print_r($categories1);echo "<hr>";
                        ...
                        foreach ($categories1 as $c) {
                            $a_new = [];
                            $a_new['searchkeyword'] = $c->searchkeyword;
                            $a_new['name'] = $c->name;
                            $a_new['rename_folder'] = $c->rename_folder;
                            $a_new['custom_text'] = $c->custom_text;
                            $a_new['parent_id'] = $last_ins_id1;
                            $a_new['folder_user_id'] = $c->folder_user_id;
                            $a_new['public_personal'] = $c->public_personal;
                            $a_new['parent_url'] = $c->parent_url;
                            $a_new['icon'] = $c->icon;
                            $a_new['short_order'] = $c->short_order;
                            $a_new['priority'] = $c->priority;
                            $a_new['status'] = $c->status;
                            $a_new['keyword'] = $c->keyword;
                            $a_new['delete_status'] = $c->delete_status;

                            $a_new['sale_price'] = $c->sale_price;
                            $a_new['paid_music_images'] = $c->paid_music_images;
                            $a_new['is_personal_library'] = $c->is_personal_library;
                            $a_new['personal_library_folder'] = $c->personal_library_folder;
                            $CI->db->insert('categories', $a_new);
                            $last_ins_id2 = $CI->db->insert_id();
                            //echo $last_ins_id;die;
                            copy_folder_database_data_gaurav($c->id, $last_ins_id2);
                        }
                        //}
                    }
                }
                // echo $categories->id;
                $a_new = [];
                $a_new['searchkeyword'] = $categories->searchkeyword;
                $a_new['name'] = $categories->name;
                $a_new['rename_folder'] = $categories->rename_folder;
                $a_new['custom_text'] = $categories->custom_text;
                $a_new['parent_id'] = $desired_id;
                $a_new['folder_user_id'] = $categories->folder_user_id;
                $a_new['public_personal'] = $categories->public_personal;
                $a_new['parent_url'] = $categories->parent_url;
                $a_new['icon'] = $categories->icon;
                $a_new['short_order'] = $categories->short_order;
                $a_new['priority'] = $categories->priority;
                $a_new['status'] = $categories->status;
                $a_new['keyword'] = $categories->keyword;
                $a_new['delete_status'] = $categories->delete_status;

                $a_new['sale_price'] = $categories->sale_price;
                $a_new['paid_music_images'] = $categories->paid_music_images;
                $a_new['is_personal_library'] = $categories->is_personal_library;
                $a_new['personal_library_folder'] = $categories->personal_library_folder;
                $this->db->insert('categories', $a_new);
                ...
                copy_folder_database_data_gaurav($categories->id, $last_ins_id);
    ...
    public function keywordsearch($home_search = '', $home_search_file = '')
    {
        if (is_numeric($home_search)) {
            $home_search = 'nocomposer';
            $home_search_file = strtolower(trim(strip_tags(str_replace('%20', ' ', $home_search_file))));
        }

        $page_url = rtrim(current_url(), "/");

        $nocomposer = '';
        if ($home_search == 'nocomposer') {
            $nocomposer = 'nocomposer';
            $home_search = '';
        }

        $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
        $this->session->unset_userdata('search_music_for_sale');
        $this->session->unset_userdata('search_list_ids');
        $this->session->unset_userdata('root_folders_ids');
        $this->session->unset_userdata('verify_folder_name_id');
        $data['home_search'] = trim($this->input->post('home_search'));
        $data['home_search_file'] = trim($this->input->post('home_search_file'));



        if ($data['home_search'] != '' || $data['home_search_file'] != '') {
            $data['search_music_for_sale'] = $this->input->post('search_music_for_sale');
            $this->session->set_userdata('search_music_for_sale', $data['search_music_for_sale']);
            if ($data['home_search'] != '' && $data['home_search_file'] != '') {
                $data['objTree_1'] = $this->Comp_model->get_keywordsearch(strtolower($data['home_search']), strtolower($data['home_search_file']));
            } elseif ($data['home_search'] != '') {
                $data['objTree_1'] = $this->Comp_model->get_keywordsearch(strtolower($data['home_search']));
            } elseif ($data['home_search_file'] != '') {
                $data['objTree_1'] = $this->Comp_model->get_keywordsearch_file(strtolower($data['home_search_file']));
            }
        } elseif ($home_search == 'guitar') { //********************$_GET url Value**************************

            $data['objTree_1'] = $this->Comp_model->get_keywordsearch_file('guitar');
            $data['home_search_file'] = 'guitar';
        } elseif (($home_search != '' && $home_search_file != '') || (($nocomposer == 'nocomposer' && $home_search_file != ''))) {
            $data['home_search'] = $home_search;
            $data['home_search_file'] = $home_search_file;
            if ($nocomposer == 'nocomposer') {
                $data['objTree_1'] = $this->Comp_model->get_keywordsearch_file(strtolower($data['home_search_file']));
            } else {
                $data['objTree_1'] = $this->Comp_model->get_keywordsearch(strtolower($home_search), strtolower($home_search_file));
            }
        } elseif ($home_search != '') {
            $data['home_search'] = $home_search;
            $data['objTree_1'] = $this->Comp_model->get_keywordsearch(strtolower($data['home_search']));
            ...
            $data['home_search'] = '';
            $data['home_search_file'] = '';
            $data['objTree_1'] = array();
        }
        ...
        $data['search_count'] = 0;
        $data['search_one_child'] = 0;
        $data['search_one_parent'] = 0;
        if (count($data['objTree_1']) == 1) { //*************************Used for auto click if record is 1;
            $data['search_count'] = 1;
            $data['search_one_child'] = $data['objTree_1'][0]->id;
            $data['search_one_parent'] = $data['objTree_1'][0]->parent_id;
        }
        $control_id = 0;

        foreach ($data['objTree_1'] as $key_parent_key => $value) {
            $root_ids = $this->Comp_model->getDirNamesById_keywordsearch($value->id, $control_id, 2);
            $a = (int)$root_ids[0];
            $b = (int)$root_ids[1];
            $c = (int)$root_ids[2];
            $root_path_array[$a][] = $b;
            $root_path_array[$a][$b][] = $c;
        }

        $this->session->set_userdata('search_list_ids', $root_path_array);
        $data['objTree'] = $root_path_array;
        ...
        $data['title'] = 'Manage Libraries';
        //  echo "<pre>";print_r($data);die;
        _adminLayout('keywordsearch', $data);
        //  return $this->load->view('keywordsearch',$data);
        //  _frontLayout('keywordsearch', $data);
    }

    function recursive_array_search_gaurav($needle, $haystack)
    {

        //echo "<pre>";print_r($needle);print_r($haystack);die;
        foreach ($haystack as $key => $value) {
            $current_key = $key;
            if ($needle === $value or (is_array($value) && $this->recursive_array_search_gaurav($needle, $value) !== false)) {
                return true;
            }
        }
        return false;
    ...
    function recursive_array_search_parent_gaurav($needle, $haystack, $main_search_data)
    {

        // foreach($haystack as $key=>$value) {
        //   $current_key=$key;
        //   if(in_array($main_search_data,$key) ) {
        //         return true;
        //     }else{
        //         if(is_array($value)){
        //             $this->recursive_array_search_parent_gaurav($needle,$value,$main_search_data);
        //         }
        //     }
        // }
        return false;
    ...
    public function ajax_child_data_keyword()
    {


        function keyword_filter_for_unset($parent_id)
        {
            $loop = 0;
            $unset_keyword = array(35552, 34630); //*************** to display all child folders for specific folders
            if (in_array($parent_id, $unset_keyword)) {
                $loop++;
            }
            return $loop;
        }

        //$ajax_search_file_folder_string_matched=array(35115,33011,44791,44797,44803,44808,44812,44819,44824,44829,44832,44835,44836);
        //echo "<pre>";print_r($this->session->userdata("search_list_ids"));die;
        $file_folder_search = 0;
        $only_file_search = 0;


        $this->session->set_userdata('permission_type', 'public');
        $parent_id = $this->input->post('parent_id');
        $data['search_level'] = $this->input->post('search_level');
        $data['permision_logs'] = $this->input->post('permision_logs');
        //print_r($this->input->post());
        // if($this->input->post('ajax_search') || $this->input->post('ajax_search_file')){
        //      $ajax_search=strtolower($this->input->post('ajax_search'));
        //      $ajax_search_file=strtolower($this->input->post('ajax_search_file'));
        // }else{
        //     $ajax_search='';
        //     $ajax_search_file='';
        // }
        ...
        $post_keyword_search['ajax_search'] = strtolower(trim($this->input->post('ajax_search')));
        //echo "<br/>";
        $post_keyword_search['ajax_search_file'] = strtolower(trim($this->input->post('ajax_search_file')));


        // $post_keyword_search['ajax_search']='Brahms';
        //        $post_keyword_search['ajax_search_file']='Piano-4-hands ';
        //        $parent_id=1;

        if ($post_keyword_search['ajax_search'] != '' || $post_keyword_search['ajax_search_file'] != '') {

            // if($post_keyword_search['ajax_search']=='eroica'){
            //  $data['ajax_search']=$post_keyword_search['ajax_search'];
            //  $data['ajax_search_file']=$post_keyword_search['ajax_search'];   
            // }else{
            $data['ajax_search'] = $post_keyword_search['ajax_search'];
            $data['ajax_search_file'] = $post_keyword_search['ajax_search_file'];
            //}     
            ...
            if ($data['ajax_search'] != '' && $data['ajax_search_file'] != '') {
                $file_folder_search = 1;
                //echo "hello";
                if ($parent_id != 1 && $parent_id != 132 && $parent_id != 1321) {
                    //echo 'true';
                    $data['objTree_1'] = $this->Comp_model->get_keywordsearch_file(strtolower($data['ajax_search_file']), $parent_id);
                } else {
                    $data['objTree_1'] = $this->Comp_model->get_keywordsearch(strtolower($data['ajax_search']), strtolower($data['ajax_search_file']));
                }
            } elseif ($data['ajax_search'] != '') {
                /************************unset keyword for specif folder like 'Beethoven '*******************/
                $unset_searching = keyword_filter_for_unset($parent_id);
                //echo strtolower($data['ajax_search_file']);
                /************************unset keyword for specif folder like 'Beethoven ' end*******************/
                if ($unset_searching == 1 && strtolower($data['ajax_search']) == 'beethoven') {
                    $data['objTree_1'] = array(); //*****************stop searching******************
                } else {
                    $data['objTree_1'] = $this->Comp_model->get_keywordsearch(strtolower($data['ajax_search']));
                }
            } elseif ($data['ajax_search_file'] != '') {
                $only_file_search = 1;
                $data['objTree_1'] = $this->Comp_model->get_keywordsearch_file(strtolower($data['ajax_search_file']), $parent_id);
            }
        }
        $control_id = 0; //$parent_id;

        // echo count($data['objTree_1']);
        // echo "<pre>";
        // print_r($data['objTree_1']);
        // die;
        $main_search_data = [];
        foreach ($data['objTree_1'] as $key => $value) {
            $main_search_data[] = $value->id;
            $root_ids_folders = $this->Comp_model->getDirNamesById_all_parent_folders($value->id, 0);
            foreach ($root_ids_folders as $key => $value) {
                $filter_all_parent_ids[] = $value;
            }
        }
        ...
        // echo "<pre>";
        // print_r($data['objTree_1']);print_r($root_ids);die;
        foreach ($root_ids as $key => $value) {
            //$root_ids_child=$this->Comp_model->getDirNamesById_keywordsearch($value->id,$control_id);
            if ((int)$value[0] == $parent_id || (int)$value[1] == $parent_id || (int)$value[2] == $parent_id || (int)$value[3] == $parent_id || (int)$value[4] == $parent_id || (int)$value[5] == $parent_id || (int)$value[6] == $parent_id || (int)$value[7] == $parent_id || (int)$value[8] == $parent_id) {
                ...
                $c = (int)$value[$start_key + 2];
                $d = (int)$value[$start_key + 3];
                // $e=(int)$root_ids[$start_key+4];      
                $root_path_array[$a][] = $b;
                //$root_path_array[$a][$b][]=$c;
                // $root_path_array[$a][$b][$c][]=$d;
                // $root_path_array[$a][$b]$c][$d][]=$e;
            }
        }
        $root_path_array_filter = array();
        //echo "<pre>";
        // print_r($root_path_array);die;
        foreach ($root_path_array[$parent_id] as $key => $value) {
            if ($value > 0) {
                $root_path_array_filter[] = $value;
            }
        ...
        // print_r($root_path_array_filter);
        // die;

        $search_list_ids = $this->session->userdata('search_list_ids')[$parent_id];
        // $url = $this->Category_model->getDirNamesById($parent_id);
        // $save['parent_url'] = $url;
        // $where['id']=$parent_id;
        // $table_name='categories';
        //  $this->Category_model->update_folder_data($table_name,$where,$save);////update folder path
        // $save1['file_path'] = $url;
        // $where1['cat_id']=$parent_id;
        // $table_name1='category_image';
        //  $this->Category_model->update_folder_data($table_name1,$where1,$save1);////update file path
        ...
        //echo "geu";print_r($root_path_array[$parent_id]);die;
        //$data['objTree'] = $this->Comp_model->get_keywordsearch($ajax_search,$ajax_search_file,$parent_id,$folder_id);
        if (count($root_path_array[$parent_id])) {
            $data['objTree'] = $this->Comp_model->get_folders_by_ids($root_path_array_filter, $parent_id);
        } elseif (count($root_path_array[$parent_id]) == 0 && ($file_folder_search == 0 && $only_file_search == 0)) {
            $data['objTree'] = $this->Comp_model->getCategoryTreeData_ajax($parent_id);
        }
        //echo 'k'.$data['ajax_search_file'];
        //$data['verify_parent_name'] = $this->Comp_model->get_keywordsearch_verify('',$parent_id);

        if ($data['ajax_search_file'] != '') {
            //echo "hello";die();
            $data['verify_folder_name'] = $this->Comp_model->get_keywordsearch_verify(strtolower($data['ajax_search_file']), $parent_id);
            // print_r($data['verify_folder_name']);
            $this->session->set_userdata('verify_folder_name_id', $data['verify_folder_name'][0]->id);
        } else {
            $data['verify_folder_name'] = array();
        ...
        if (count($data['verify_folder_name']) > 0 && $data['ajax_search_file'] != '') {
            //echo "gaurav";
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 1);
        } elseif ($data['verify_parent_name'][0]->parent_id == 1 || $data['verify_parent_name'][0]->parent_id == 132 || $data['verify_parent_name'][0]->parent_id == 1321 || $data['verify_parent_name'][0]->parent_id == 13763 || $data['verify_parent_name'][0]->parent_id == 12619 || $data['verify_parent_name'][0]->parent_id == 12522) {
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
        } elseif ($data['search_level'] < $this->session->userdata('searching_level')) {
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
        } elseif ($parent_id == 1 || $parent_id == 132 || $parent_id == 1321 || $parent_id == 13763 || $parent_id == 12522 || $parent_id == 12619) {
            $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
        } else {
            $ar_gaurav = $this->session->userdata("search_list_ids");
            if ($this->recursive_array_search_gaurav($parent_id, $ar_gaurav)) {
                $this->session->set_userdata('ajax_search_file_folder_string_matched', 0);
            }
            if ($this->recursive_array_search_parent_gaurav($parent_id, $ar_gaurav, $main_search_data)) {
                $this->session->set_userdata('ajax_search_file_folder_string_matched', 1);
            }
            //$this->session->set_userdata('ajax_search_file_folder_string_matched',0);
        }
        ...
        //echo $this->session->userdata('ajax_search_file_folder_string_matched');
        if (($this->session->userdata('ajax_search_file_folder_string_matched') > 0) && ($parent_id != 1 && $parent_id != 132 && $parent_id != 1321)) {
            // echo "hello";
            $data['objTree'] = $this->Comp_model->getCategoryTreeData_ajax($parent_id);
        }

        //print_r($data['objTree']);die;
        if (count($data['objTree']) == 1) { //*************************Used for auto click if record is 1;
            $data['search_count'] = 1;
            $data['search_one_child'] = $data['objTree'][0]->id;
            $data['search_one_parent'] = $data['objTree'][0]->parent_id;
        }
        ...
        // foreach ($data['objTree'] as $key => $value) {
        //         $root_ids=$this->Comp_model->getDirNamesById_keywordsearch($value->id,$control_id);
        //         $root_path_array[$root_ids[0]]=$root_ids;
        //           }
        // //          echo "<pre>";
        //           $this->session->set_userdata('search_list_ids',$root_path_array);
        ...
        $data['parent_id_tree'] = $parent_id;
        // $data['ajax_search'] = $ajax_search;
        //  $data['ajax_search_file'] = $ajax_search_file;
        // if($ajax_search!='' && count($data['objTree'])==0){
        //     echo "No Record Found. Keyword '<strong>".$ajax_search."</strong>'";die;
        // }

        $permision_logs = $data['permision_logs'];
        $permision_logs_array = explode('_', $permision_logs);

        if ($permision_logs != '') {
            $arr_img_pdf_txt = $this->Comp_model->getImagesById_share($parent_id, $permision_logs_array);
        } else {
            $arr_img_pdf_txt = $this->Comp_model->getImagesById($parent_id, $ajax_search_file);
        }
        $folders = $data['objTree'];
        unset($data['objTree']);
        $data['objTree']['permision_logs_array'] = $permision_logs_array;
        $data['objTree']['permision_logs'] = $permision_logs;
        $data['objTree']['folders'] = $folders;
        $data['objTree']['files'] = $arr_img_pdf_txt;
        echo json_encode($data['objTree']);
        //return $this->load->view('ajax_child_tree',$data);
        die;
    }

    private function sub_cat_delete_records()
    {
        $this->load->database();
        $categories = $this->db
            ->where('delete_status', 2)
            ->where('custome_update', 0)
    ...
    private function deleteCategory($categoryId)
    {
        ...
        $savedata['delete_status'] = 2;
        $savedata['custome_update'] = 1;
        $this->db
            ->where('id', $categoryId)
            ->update('categories', $savedata);

        return true; // Return true if the category and its children were successfully deleted
    }
}
