...
<style type="text/css">
  ...
  .without_custom_text_file {
    margin-top: 7px !important;
  }

  .without_custom_text_on_filefile_delete_option_3 {
    margin-right: 0;
    margin-top: 12px;
  }
      ...
      <div class="x_title">
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <form method="post" id="dhs-form-search1" action="<?= base_url('admin_categories/move_folders') ?>" style="display: inline-block;">

              <button type="submit" id="folder-data-multiple-button" class="btn btn-info" name="move_folders" value="true" style="display:none;">Move Folders</button>
              <input type="hidden" name="ids" value="" id="folder_data_multiple_field" />
  ...
  $("#submit_custom_text").click(function() {
    ...
    $.ajax({
      ...
      data: {
        id: id,
        type: type,
        new_custom_text: new_custom_text
      },
      success: function(html) {
...
<script type="text/javascript">
  ...
  function ajax_root_folder_index(parent_id, cate_type, alphaSearch_keyword, redirect_type, parent_url) {
    ...
    $.ajax({
      ...
      data: {
        ...
        redirect_type: redirect_type,
        parent_url: parent_url,
        alphaSearch_keyword: alphaSearch_keyword
      },
      success: function(html) {
        //  }
  ...
  function json_to_html(json_data, parent_id = '') {
   ...
   <div class="child_folder_name">
      <input type="checkbox" class="folder-data-multiple" name="select_folders[]" value="` + folder_array[i].id + `" />
   ...
   <span class="custom_text_p file_cutomtxt" id="update_custom_text_` + files_array[j].id + `">` + custtext + `</span>
    ...
    if (auto_open_folder_id > 0) {
      ...
      $('html, body').animate({
        scrollTop: $("#" + auto_open_folder_id).offset().top
      }, 2000);
      console.log("gaurav")
...
<script>
  ...
  $(document).on("change", ".folder-data-multiple", function() {
    $("#folder-data-multiple-button").hide();
    flag = 0;
    ids = new Array();
    $(".folder-data-multiple").each(function() {
      if ($(this).prop("checked") == true) {
        flag = flag + 1;
        ids.push($(this).val());
      }
    ...
    if (flag > 0) {
      ...
      $("#folder-data-multiple-button").show();
      $("#folder-data-multiple-button").html("Move " + flag + " Folders");
      $("#folder_data_multiple_field").val(ids.join());
    }
  });
</script>