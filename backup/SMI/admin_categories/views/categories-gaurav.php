...
if (!in_array($parent_id, $this->session->userdata('destination_ids')))
  {
 unset($_SESSION['destination_ids']);
 $destination_ids_json=json_encode(array(0));               
  }else{
    $destination_ids_json=json_encode($this->session->userdata('destination_ids'));
  }
...
<div class="clearfix"></div>
        ...
        <p class="tabs" style="margin: 17px 10px 0 10px;">
            ...
            <a href="<?php echo site_url(); ?>admin_categories/admin_s_d" <?php if($parent_id==14155){ ?>style="background-color: #666666; color: #fff;" <?php } ?>>S.D</a>
      ...
      //if(new_custom_text!=''){
            ...
            $.ajax({
                  ...
                  success: function(html){
                    ...
                    $('.tooltip_a_'+id).css({"visibility": "hidden"});
                    }
...
function view_child_data_function (id){
        ...
        $.ajax({
          ...
          cache: false,
          data:{parent_id:parent_id,redirect_type:redirect_type,parent_url:parent_url},
          success: function(html){
                      child_data_visible(parent_id,class_open);
...
if(folder_array[i].parent_id == 132){
   ...
   <div class="child_folder_name">
      ...
      <div class="naoTooltip-wrap" style="top:0px;" id="naoTooltip_`+folder_array[i].id+`" onmouseout="naoTooltip_hide(this.id)" onmouseover="naoTooltip_show(this.id)">
...
if(ext=="pdf" || ext=="PDF"){
 ...
 var ext_icon='<?php echo IMAGE_PATH;?>txt.png'; 
        ...
        html_data+=`<div style="border-bottom: 1px solid;" id="header_row_`+files_array[j].id+`" class="file_delete_`+files_array[j].id+` child_pdf_files" data-parent_id="`+files_array[j].parent_id+`">
   ...
   <img src="`+ext_icon+`" style="height: 24px;"> `+file_name+`</a>
   <a class="tooltip tooltip_a_`+files_array[j].id+`" style="visibility: hidden; ">&nbsp;&nbsp; <i class="fas fa-info-circle" style="font-size: 20px; margin-top: 3px;" aria-hidden="true"></i><span class="tooltiptext tooltip_a_span_`+files_array[j].id+`"></span></a> 
...
html_data+=`<div class="naoTooltip nt-left nt-small dd-handle  naoTooltip_show_`+files_array[j].id+` naoTooltip_menu_tabs_child" style="opacity: 1; left: 63px; top: -100px; visibility: hidden;">
   ...
   html_data+=`</div>
   <span class="custom_text_p file_cutomtxt" id="update_custom_text_`+files_array[j].id+`"></span>
</div>`;
...
setTimeout(function(){ 
      ajax_root_folder_index('<?php echo $parent_id;?>','<?php echo $cate_type;?>','<?php echo $alphaSearch_keyword;?>','<?php echo $redirect_type;?>'); 
    }, 500);
